...
public class BoardService {
    ...
    @Transactional
    public Page<BoardResponseDto> getBoardListByUpdate(Pageable pageable,Authentication authentication){
        if(authentication != null && authentication.getPrincipal() != "anonymousUser"){
            return boardRepository.findAllByBoardReplyCreationTime(pageable).map(board->new BoardResponseDto(board,userLikeBoardRepository.findByBoardIdAndUserId(board.getId(),userRepository.findByUsername(authentication.getName()).getId())));
        }
        return boardRepository.findAllByBoardReplyCreationTime(pageable).map(BoardResponseDto::new);
    }
}
