...
public class BoardController {
    ...
    @ResponseBody
    public ResponseEntity<Page<BoardResponseDto>> getBoardlistByUpdate(@PageableDefault Pageable pageable){
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        Pageable modifiedPageable = PageRequest.of(pageable.getPageNumber(), defaultPageSize);
        Page<BoardResponseDto> boardList=boardService.getBoardListByUpdate(modifiedPageable,authentication);
        return ResponseEntity.ok(boardList);
    }
}
